...
#         x = F.relu(self.fc_3(x))
        ...
        prob = torch.distributions.Normal(mu, var)
        sample = prob.sample()
...
#         x = F.relu(self.fc_3(x))
    ...
    def make_batch(self):
        ...
        s,a,r,s_prime,done_mask, log_prob_a = torch.tensor(s_lst, dtype=torch.float), torch.tensor(a_lst), \
                                          torch.tensor(r_lst), torch.tensor(s_prime_lst, dtype=torch.float), \
                                          torch.tensor(done_lst, dtype=torch.float), torch.tensor(log_prob_a_lst)
        self.data = []
    ...
    for n_epi in range(N):
        ...
        while not done:
            for t in range(T_horizon):
                ...
                model.put_data((s, a, r/100.0, s_prime, log_prob, done))
                s = s_prime
